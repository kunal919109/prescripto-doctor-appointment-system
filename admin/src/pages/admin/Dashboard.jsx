...
const StatCard = ({ icon, label, value, error, loading }) => (
  <div className="bg-white rounded-2xl border border-border p-5 flex items-center gap-4 shadow-sm">
    ...
    <div className="min-w-0">
      ...
      {error ? (
        <p className="text-error text-sm mt-0.5">⚠ Failed to load</p>
      ) : loading ? (
  ...
  const fetchDashboard = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setCardErrors({ doctors: false, patients: false, appointments: false });
      } else {
        toast.error(data.message || 'Failed to load dashboard');
        setCardErrors({ doctors: true, patients: true, appointments: true });
      }
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-text-secondary text-sm mt-1">Platform overview at a glance</p>
      </div>
